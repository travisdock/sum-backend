    ...
    def v2charts
        ...
        stats = {
          self.year_view => {
            ...
            "Monthly Averages" => avg_cat_month
          }
        }
        ...
        # Group entries by month
        monthly_totals = self.entries.where(date: year_range).group_by{ |entry| entry.date.beginning_of_month.strftime("%B") }
        # Group month's entries by category
        monthly_totals.transform_values! { |entries| entries.group_by{|e| e.category_name } }
        # Change individual entries into sums by category and month
        monthly_totals.each do |month, categories|
          categories.transform_values! { |entries| entries.map(&:amount).inject(0, &:+) }
        end
        ...
        monthly_totals.each do |month, categories|
            monthly_totals[month] = { "Category Totals" => categories }
        end
        ...
        monthly_stats.each do |month, info|
          ...
          monthly_stats[month]['Stats']["Total Income"] = income_by_month[month] || 0
          monthly_stats[month]['Stats']["Total Expense"] = expense_by_month[month] || 0
          monthly_stats[month]['Stats']["Profit/Loss"] = monthly_stats[month]['Stats']["Total Income"] - monthly_stats[month]['Stats']["Total Expense"]
        end
